...
export function NavGroup({ name, items, expandedPath, onToggle }: NavGroupProps) {
			...
			<CollapsibleContent>
				<ul className="flex w-full flex-col gap-1">
					{items.map((item, index) => (
						<NavList key={item.title || index} data={item} depth={1} expandedPath={expandedPath} onToggle={onToggle} />
					))}
				</ul>
			</CollapsibleContent>
		</Collapsible>
